...
def generate_beam(model: ClipCaptionModel, tokenizer, beam_size: int = 5, prompt=None, embed=None,
    ...
    model.eval()
    #print("use generate_beam")
    stop_token_index = tokenizer.eos_token_id  # 使用 eos_token_id
    tokens = None
    scores = None
    ...
    with torch.no_grad():
        ...
        for i in range(entry_length):
            ...
            is_stopped = is_stopped + next_tokens.eq(stop_token_index).squeeze()  # 停止生成
            if is_stopped.all():
                break
    ...
    output_texts_strip = clean_generated_text(output_texts)
    return output_texts_strip
...
def generate2(
    ...
    return clean_generated_text(generated_list[0])
